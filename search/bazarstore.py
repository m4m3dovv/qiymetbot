"""
BazarStore (bazarstore.az) searcher
Azerbaijan online grocery store
"""
...
class BazarStoreSearcher(BaseSearcher):
    marketplace_name = "BazarStore"
    marketplace_emoji = "🛒"
    is_local = True
    ...
    async def search(self, query: str, barcode: Optional[str] = None) -> list[SearchResult]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # BazarStore API / search page
                search_url = f"https://bazarstore.az/az/search?q={query}"
                headers = {
                ...
                for prod in products[:10]:
                    try:
                        ...
                        if link and not link.startswith("http"):
                            link = f"https://bazarstore.az{link}"
